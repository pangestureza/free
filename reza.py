...
def log_replace(value, status='Sabar Bos biar ganteng', color='[R1]'):
...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='Sabar Bos', color='[P1]'):
    ...
    def run(self):
        try:
            ...
            self.log('Kalo Konek Anda Tamvan !'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
